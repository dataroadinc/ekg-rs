...
impl<'a, W: 'a + Write> Streamer<'a, W> {
    ...
    fn evaluate(mut self) -> Result<Self, ekg_error::Error> {
        ...
        let stream = Box::new(rdfox_sys::COutputStream {
        ...
        let result = rdfox_sys::database_call! {
            "Evaluating a SPARQL statement",
            rdfox_sys::CDataStoreConnection_evaluateStatement(
                connection_ptr,
                statement_text.as_ptr(),
                statement_text_len,
                c_params.cast_const(),
                stream_raw_ptr as *const rdfox_sys::COutputStream,
    ...
    fn connection_ptr(&self) -> *mut rdfox_sys::CDataStoreConnection { self.connection.inner }
}
